...
function TalkToContent() {
  ...
  const [loadingDouble, setLoadingDouble] = useState(true);
  ...
  useEffect(() => {
    ...
    async function loadDouble() {
      try {
        ...
        // Essayer de charger par share_slug d'abord
        let response = await fetch(`/api/double-ia/get-by-slug?slug=${username}`);

        // Si pas trouvé par slug, essayer par ID
        if (!response.ok) {
          response = await fetch(`/api/double-ia/get?id=${username}`);
        }

        if (response.ok) {
          const data = await response.json();
          setDoubleInfo(data.double);

          // Charger l'avatar du propriétaire
          if (data.double?.user_id) {
            const userResponse = await fetch(`/api/user/stats?userId=${data.double.user_id}`);
            if (userResponse.ok) {
              const userData = await userResponse.json();
              if (userData.user?.avatar_url) {
                setOwnerAvatar(userData.user.avatar_url);
              }
  ...
  useEffect(() => {
    if (!doubleInfo?.id) return;

    async function loadMessages() {
      try {
        const messagesResponse = await fetch(
          `/api/double-ia/messages?doubleId=${doubleInfo.id}${userId ? `&userId=${userId}` : ''}`
        );
        ...
        if (messagesResponse.ok) {
          ...
          if (messagesData.messages && messagesData.messages.length > 0) {
            ...
            const welcomeMessage: Message = {
              id: 'welcome',
              role: 'assistant',
              content: `Salut ! Je suis le double IA de ${ownerName || 'cette personne'}. Pose-moi des questions ! 😊`,
              timestamp: new Date(),
            };
    ...
    loadMessages();
  }, [doubleInfo?.id, userId, ownerName]);
  ...
  const sendMessage = async () => {
    if (!input.trim() || isLoading || !doubleInfo?.id) return;
  ...
  // Double IA non trouvé
  if (notFound || !doubleInfo) {
    return (
      <div className="h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 mb-4">Double IA introuvable</h1>
          <p className="text-gray-600 mb-6">Ce double IA n'existe pas ou n'est pas partagé publiquement.</p>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <h2 className="font-semibold text-gray-900">
                Double IA de {ownerName || 'Utilisateur'}
              </h2>
